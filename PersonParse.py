...
def getName(name):
    ...
    return str(uuid4()).replace("-", "")[:8] + "~" + name.replace("~", "-")
...
def run(filename):
    ...
    with open(output_file, 'w', encoding='utf-8') as out_file:
        with open('error_lines.txt', 'w', encoding='utf-8') as error_file:
            counter = 0
            for line in lines:
                if  line[:6] == '!&8%$#':
